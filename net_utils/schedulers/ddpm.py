...
class DDPMScheduler(Scheduler):
    ...
    def set_timesteps(
        ...
        ), "{self.__class__} can only do one step at a time like in the forward process."

        self.num_inference_steps = self.num_train_timesteps
        self.timesteps = self.get_timesteps(1000).to(device)

    def _get_mean(
        self, timestep: int, x_0: torch.Tensor, x_t: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        x_0_coefficient = (
            alpha_prod_t_prev.sqrt() * self.betas[timestep] / (1 - alpha_prod_t)
        )
    ...
    def _get_variance(
        self, timestep: int, predicted_variance: torch.Tensor | None = None
    ) -> torch.Tensor:
        """
