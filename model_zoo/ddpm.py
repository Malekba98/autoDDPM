...
class AnomalyMap:
    ...
    def dilate_masks(self, masks):
        ...
        for i in range(masks.shape[0]):
            ...
            dilated_mask = (
                torch.from_numpy(dilated_mask).to(masks.device).unsqueeze(dim=0)
            )
    ...
    def lpips_loss(self, anomaly_img, ph_img, retPerLayer=False):
        ...
        for batch_id in range(anomaly_img.size(0)):
            lpips = self.l_pips_sq(
                2 * anomaly_img[batch_id : batch_id + 1, :, :, :] - 1,
                2 * ph_img[batch_id : batch_id + 1, :, :, :] - 1,
                normalize=True,
                retPerLayer=retPerLayer,
            )
            if retPerLayer:
                lpips = lpips[1][0]
...
class DDPM(nn.Module):
    ...
    def get_anomaly(
        ...
        assert (
            method == "anoDDPM" or method == "autoDDPM"
        ), "Method should be either anoDDPM or autoDDPM"
        if method == "anoDDPM":
            x_rec, _ = self.sample_from_image(
                inputs, noise_level=noise_level, verbose=verbose
            )
            x_rec = torch.clamp(x_rec, 0, 1)
            anomaly_maps = np.abs(
                inputs.cpu().detach().numpy() - x_rec.cpu().detach().numpy()
            )
            anomaly_scores = np.mean(anomaly_maps, axis=(1, 2, 3), keepdims=True)
            x_rec_dict = {"x_rec": x_rec}
        else:
            anomaly_maps, anomaly_scores, x_rec_dict = self.get_autoDDPM_anomaly(
                inputs=inputs,
