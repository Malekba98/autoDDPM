...
class DLConfigurator(object):
    ...
    def __init__(self, config_file, log_wandb=False):
        ...
        if self.dl_config is None:
            print(
                "[Configurator::init] ERROR: Invalid configuration file. Configurator will exit..."
            )
            return
    ...
    def start_palette_editing(self, global_model):
        data = self.load_data(self.dl_config["trainer"]["data_loader"], train=True)
    ...
    def start_evaluations(self, global_model):
        ...
        for idx, dst_name in enumerate(self.dl_config["downstream_tasks"]):
            logging.info(
                "[Configurator::eval]: ################ Starting downstream task nr. {}/{} -- {}-- ################".format(
                    idx + 1, nr_tasks, dst_name
                )
            )
    ...
    def load_data(data_loader_config, train=True):
        ...
        data_loader_module = import_module(
            data_loader_config["module_name"], data_loader_config["class_name"]
        )
