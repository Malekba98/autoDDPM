...
def check_config_file(config_file):
    ...
    for mdx, module in enumerate(modules):
        ...
        assert "module_name" in config_file[module].keys(), (
            base_message
            + 'Config file should contain keyword "'
            + module
            + '": module_name!'
        )
    ...
    assert "optimizer_params" in training_params.keys(), (
        base_message
        + "Please use the keywords [optimizer_params] in the training_params dictionary"
    )
    ...
    assert "loss" in training_params.keys(), (
        base_message
        + "Please use the keywords [loss] in the optimizer_params dictionary"
    )
