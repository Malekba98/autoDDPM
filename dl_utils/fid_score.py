...
from dl_utils.inception import InceptionV3
...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        print("files shape", self.files.shape)
        print("shape of element", self.files[i].shape)
        img = self.files[i] * 255
        return img


def get_activations_from_tensor(
    tensor, model, batch_size=50, dims=2048, device="cpu", num_workers=1, is_path=True
):
    model.eval()

    pred_arr = np.empty((tensor.shape[0], dims))
...
def get_activations(
    files, model, batch_size=50, dims=2048, device="cpu", num_workers=1, is_path=True
):
    ...
    dataset = (
        ImagePathDataset(files, transforms=TF.ToTensor())
        if is_path
        else ImageDataset(files, transforms=TF.ToTensor())
    )
...
def calculate_activation_statistics(
    ...
    """
    if TENSOR:
        act = get_activations_from_tensor(
            files, model, batch_size, dims, device, num_workers
        )
    else:
        act = get_activations(
            files, model, batch_size, dims, device, num_workers, is_path
        )
...
def compute_statistics_of_set(images, model, batch_size, dims, device, num_workers=1):

    files = images
    m, s = calculate_activation_statistics(
        files, model, batch_size, dims, device, num_workers, is_path=False
    )
...
def calculate_fid_given_images(images, batch_size, device, dims, num_workers=1, bootstrap=False):
    ...
    if bootstrap:
        ...
        m1, s1 = compute_statistics_of_set(
            images[0], model, batch_size, dims, device, num_workers
        )
        ...
        for _ in range(10):
            ...
            sampled_tensor = images[1][indices]

            print(sampled_tensor.shape)  # Prints

            m2, s2 = compute_statistics_of_set(
                sampled_tensor, model, batch_size, dims, device, num_workers
            )
            fid_value = calculate_frechet_distance(m1, s1, m2, s2)
        ...
        m2, s2 = compute_statistics_of_set(
            images[1], model, batch_size, dims, device, num_workers
        )
        fid_value = calculate_frechet_distance(m1, s1, m2, s2)
