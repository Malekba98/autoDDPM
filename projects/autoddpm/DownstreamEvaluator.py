...
class PDownstreamEvaluator(DownstreamEvaluator):
    ...
    def __init__(
        self, name, model, device, test_data_dict, checkpoint_path, global_=True
    ):
    ...
    def thresholding(self, global_model):
        ...
        for sort_idx in sorted_idxs:
            ...
            if fpr_ <= 5:
                ...
                break
        print(
            f"Th_1: [{th_1}]: {fpr_1} || Th_2: [{th_2}]: {fpr_2} || Th_5: [{best_th}]: {best_fpr}"
        )
        return best_th
    ...
    def _log_visualization(self, to_visualize, i, count):
        ...
        for idx, dict in enumerate(to_visualize):
            ...
            axarr[idx].axis("off")
            tensor = (
                dict["tensor"][i].cpu().detach().numpy().squeeze()
                if isinstance(dict["tensor"], torch.Tensor)
                else dict["tensor"][i].squeeze()
            )
            axarr[idx].imshow(
                tensor,
                cmap=dict.get("cmap", "gray"),
                vmin=dict.get("vmin", 0),
                vmax=dict.get("vmax", 1),
            )
        diffp.set_size_inches(len(to_visualize) * 4, 4)

        wandb.log(
            {
                f"Anomaly_masks/Example_Atlas_{count}": [
                    wandb.Image(diffp, caption="Atlas_" + str(count))
                ]
            }
        )
    ...
    def find_mask_size_thresholds(self, dataset):
        ...
        plt.axvline(
            ...
            color="r",
            linestyle="--",
            label=f"25th Percentile: {lower_tail_threshold}",
        )
        plt.axvline(
            upper_tail_threshold,
            color="g",
            linestyle="--",
            label=f"75th Percentile: {upper_tail_threshold}",
        )
        ...
        wandb.log(
            {
                "Anomaly/Mask sizes1": [
                    wandb.Image(Image.open(buf), caption="Mask Sizes")
                ]
            }
        )
    ...
    def pathology_localization(self, global_model, th):
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            for idx, data in enumerate(dataset):
                ...
                anomaly_maps, anomaly_scores, x_rec_dict = self.model.get_anomaly(
                    x,
                ...
                if self.model.method == "autoDDPM":
                    to_visualize = [
                        ...
                        {
                            "title": "Coarse Rec.",
                            "tensor": x_rec_dict["x_rec_orig"],
                            "cmap": "gray",
                            "vmax": 1,
                        },
                        {
                            "title": f"Coarse Anomaly Map {x.max():.3f}",
                            "tensor": x_rec_dict["x_res_orig"],
                            "cmap": "plasma",
                            "vmax": 0.3,
                        },
                    ]
                    ...
                    to_visualize.append(
                        {
                            "title": "Bin mask",
                            "tensor": x_rec_dict["mask"],
                        }
                    )
                    to_visualize.append(
                        {
                            "title": "Stitched",
                            "tensor": x_rec_dict["stitch"],
                        }
                    )
                    to_visualize.append(
                        {
                            "title": "Final inpainted image",
                            "tensor": x_rec_dict["x_rec"],
                        }
                    )
                    to_visualize.append(
                        {
                            "title": f"Final Anomaly Map {anomaly_maps.max():.3f}",
                            "tensor": anomaly_maps,
                            "cmap": "plasma",
                            "vmax": 0.4,
                        }
                ...
                for i in range(len(x)):
                    if (
                        torch.sum(masks[i][0]) > self.model.threshold_low
                        and torch.sum(masks[i][0]) <= self.model.threshold_high
                    ):  # get the desired sizes of anomalies
                        ...
                        if int(count) % 12 == 0 or int(count) in [
                            0,
                            66,
                            325,
                            352,
                        ...
                        loss_lpips = np.squeeze(
                            lpips_alex(x_i.cpu(), rec_2_i.cpu()).detach().numpy()
                        )
                        test_metrics["LPIPS"].append(loss_lpips)
                        ssim_ = ssim(
                            rec_2_i.cpu().detach().numpy(),
                            x_i.cpu().detach().numpy(),
                            data_range=1.0,
                        )
                        test_metrics["SSIM"].append(ssim_)

                    elif (
                        torch.sum(masks[i][0]) <= 1
                    ):  # use slices without anomalies as "healthy" examples on same domain
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            predictions_all = np.reshape(
                np.asarray(predictions), (len(predictions), -1)
            )  # .flatten()
            ...
            print(
                f"Labels go from {np.min(labels_all)} to {np.max(labels_all)} with mean: {np.mean(labels_all)}"
            )
            ...
            for dice_threshold in ths:
                dice = compute_dice(
                    copy.deepcopy(predictions_all),
                    copy.deepcopy(labels_all),
                    dice_threshold,
                )
            ...
            wandb.log({"Metrics/Global_highest_DICE": highest_score})
        ...
        for metric in metrics:
            ...
            fig_bp.add_trace(go.Box(y=y, x=x, name=metric, boxmean="sd"))
            title = "score"
            ...
            fig_bp.update_yaxes(
                range=[0, 1], title_text="score", tick0=0, dtick=0.1, showgrid=False
            )
